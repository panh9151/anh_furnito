...
const CartItem = ({ type, image, name, quantity, id, price, salePrice }) => {
  const DeleteIcon = () => {
    return (
      <svg
        width="20"
  ...
  return (
    <div className={clsx("d-flex w-100 gap", classes.cartItemWrapper)}>
      <div className={classes.image}>
        <img src={image} alt="" />
      </div>
      <div className={classes.content}>
        <h3>{name}</h3>
        <div className="d-flex align-items-center w-100 gap">
          <div className={classes.contentQuantity}>{quantity}</div>
          <div className={classes.contentX}>X</div>
          <div className="d-flex flex-column align-items-end">
            <div className={classes.contentPrice}>{formatNumber(price)}</div>
          </div>
        </div>
      </div>
